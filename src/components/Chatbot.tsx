...
User query: ${prompt}`;
      ...
      return 'Sorry, I\'m having trouble connecting right now. Please try again later or contact Niharika and Yash directly!';
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        {/* Main Chat Button */}
        <motion.button
          className={`w-16 h-16 bg-gradient-to-r from-orange-500 to-pink-500 rounded-full shadow-2xl hover:shadow-orange-500/50 transition-all duration-300 border-2 border-background/30 relative overflow-hidden group ${isOpen ? 'scale-0' : 'scale-100'
            }`}
          onClick={() => {
            setIsOpen(true);
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className={`fixed bottom-20 right-6 z-50 bg-gradient-to-br from-background via-orange-500/5 to-pink-500/5 backdrop-blur-lg shadow-2xl border border-orange-200/30 overflow-hidden ${isMinimized ? 'w-80 h-16 rounded-2xl' : 'w-80 h-96 flex flex-col rounded-t-2xl'
              }`}
          ...
          >
            ...
            {/* Messages */}
            <motion.div
              className={`overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-background/30 to-secondary/10 ${isMinimized ? 'h-0 opacity-0' : 'flex-1 opacity-100'
                }`}
              animate={{
              ...
              {messages.map((message) => (
                ...
                >
                  ...
                  >
                    ...
                    <p className={`leading-relaxed ${message.sender === 'user' ? 'text-white' : 'text-card-foreground'
                      }`}>{message.text}</p>
                  </div>
                </motion.div>
              ))}
            ...
            {/* Input */}
            <div
              className={`p-4 border-t border-border bg-muted/30 flex-shrink-0 ${isMinimized ? 'hidden' : 'block'
                }`}
            >
